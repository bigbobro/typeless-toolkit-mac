...
- `runtime-backups/`: 执行设备重置或弹窗补丁前自动保存的本地账号数据备份。不要上传。

这些文件已写入 `.gitignore`。

更新项目时不要直接删除旧目录。`accounts.json` 和 `profiles/` 不会跟随 GitHub release 或 `git pull` 上传/下载;如果换了目录,管理器会在新目录里看到一套新的空数据。迁移时把旧目录里的 `accounts.json`、`profiles/` 和 `Typeless词库主清单.csv` 一起带过去。

管理器顶部会显示运行数据备份状态:

- 「已备份」表示当前目录里的账号、profile 快照和主词库已经有本地备份。
- 「未备份」表示这些运行数据在最近一次备份后又有变化。
- 「立即备份」会备份到固定的 `runtime-backups/`,用于可靠检测状态。
- 「导出备份包」会下载一个可迁移的 JSON 备份包,适合换目录、换机器或发给同事。
