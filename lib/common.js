...
//
// 为什么不用 captureTokenCDP:那条路径为了拿到 Authorization 头必须 Page.reload 主窗口
// 再等 6 秒。注册向导每 4 秒轮询一次 /api/current,于是用户正在 Typeless 里填注册表单时
// 页面每 6-10 秒被刷掉一次 —— 而向导第二步要求的恰恰就是「在 Typeless 注册并登录」。
// 只有真正需要 token 的 /api/capture 才该走 reload 那条路。
//
