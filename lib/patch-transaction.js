...
const {
  ...
  safeName,
  hashFile,
  ensurePrivateDirectory: ensurePrivateDir,
  writePrivateJson,
} = require('./private-fs');
...
function createBackup({
  ...
  const finalDir = path.join(
    backupRoot,
    `${fileStamp(now)}-${safeName(label, 'patch')}-${versionPart}-${transactionId}`,
  );
  ...
  try {
    for (const file of files) {
      ...
      const backupName = safeName(file.name, 'patch');
...
function recoverIncompletePatchTransactionsCore({
  ...
  for (const backup of incomplete) {
    ...
    const transactionId = backup.manifest.transaction_id || safeName(path.basename(backup.dir), 'patch');
    try {
