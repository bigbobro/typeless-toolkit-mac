...
const {
  ensurePrivateDirectory: ensurePrivateDirectoryRaw,
  secureTree,
  writePrivateJson,
  hashFile,
} = require('./private-fs');
...
// 错误类型由本模块注入,实现统一在 lib/private-fs.js(严格语义:先 lstat 校验)
function ensurePrivateDirectory(dirPath) {
  return ensurePrivateDirectoryRaw(dirPath, {
    onInvalid: p => new RuntimeDataError(
      `Runtime data path is not a private directory: ${p}`,
...
function secureEntry(entryPath) {
  return secureTree(entryPath, {
    onUnsupported: (p, kind) => new RuntimeDataError(
      kind === 'symlink'
        ? `Runtime data contains a symbolic link: ${p}`
        : `Runtime data contains an unsupported entry: ${p}`,
      'RUNTIME_DATA_ENTRY_UNSUPPORTED',
      { path: p },
    ),
  });
