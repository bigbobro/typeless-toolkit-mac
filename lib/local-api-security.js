...
function createLocalApiSecurity(options = {}) {
  ...
  const headerName = SESSION_HEADER.toLowerCase();
  const port = options.port;

  return Object.freeze({
    headerName,   // 供 test/local-api-security.test.js 构造请求头,manager.js 不读
    injectHtml(html) {
      return injectSessionSecret(html, sessionSecret);
    },
