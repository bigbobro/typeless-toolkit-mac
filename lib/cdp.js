...
/**
 * CDP 抓 token 子系统 —— 连 Typeless 的渲染进程,把 Authorization 头捞出来
 *
 * 从 lib/common.js 抽出。这一块自成闭环:端口探测(down / foreign / no-target /
 * ready 四态)→ 必要时重启 → 连 WebSocket → 注入捕获脚本 → 重载页面 → 读回 token。
 *
 * 三点暗知识:
 *
 ...
 *
 * 3. **Page.reload 是有副作用的**:用户会看到 Typeless 刷新一次。这是为了触发
 *    应用启动时的鉴权请求,没有别的办法拿到那个头。
 *
