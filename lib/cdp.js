 ...
 * 连接约束:
 *
 * 1. **区分端口状态是必需的,不是防御性编程。** Electron 绑不上已被占用的端口时是
...
function createCdp({
  ...
  function selectTypelessCdpTarget(targets, options = {}) {
    ...
    for (const target of targets) {
      ...
      // 主窗口关闭后浮条仍在，并提供相同的主进程 IPC 和请求校验能力。
  ...
  //   no-target- 是 Typeless 的调试端口,但还没有可用的管理页面
  //   ready    - 是 Typeless,且主窗口、登录页或常驻浮条可连
  ...
  async function ensureApp(options = {}) {
    ...
    const restarted = first.status === 'down';
    if (restarted) {
      log('Typeless 管理端口不可达,正在以调试端口启动或重启…');
      await stopApp();
    ...
    for (let i = 0; i < attempts; i++) {
      if ((await probe()).status === 'ready') {
        return { state: 'connected', port: CDP_PORT, cdp_reachable: true, restarted };
      }
      if (i < attempts - 1) await wait(pollDelayMs);
    }
    const error = new Error(restarted
      ? `Typeless 启动后仍无法连接管理端口 ${CDP_PORT}`
      : 'Typeless 管理端口正常，但管理页面未就绪，请打开 Typeless 主窗口后重试');
    error.code = restarted ? 'CDP_START_TIMEOUT' : 'CDP_TARGET_UNAVAILABLE';
    throw error;
  }
  async function withCDP(fn) {
    ...
    if (!target) throw new Error('找不到可用的 Typeless 管理页面，请打开 Typeless 主窗口并检查管理连接');
    if (typeof WebSocket !== 'function') throw new Error('当前 Node.js 缺少 WebSocket 支持,请使用 Node.js 22+');
    const ws = new WebSocket(target.webSocketDebuggerUrl);
    let connectTimer, rejectOpen, failure;
    let id = 0; const pending = new Map();
    const fail = error => {
      failure ||= error;
      clearTimeout(connectTimer);
      if (rejectOpen) { rejectOpen(failure); rejectOpen = null; }
    ...
    ws.onerror = () => fail(new Error('连接 Typeless WebSocket 失败'));
    ws.onclose = () => fail(new Error('Typeless WebSocket 连接已关闭'));
    ws.onmessage = e => {
      let m;
      try {
        m = JSON.parse(e.data);
        if (!m || typeof m !== 'object' || Array.isArray(m)) throw new Error();
      } catch (_) { fail(new Error('Typeless CDP 响应格式无效')); return; }
      const item = pending.get(m.id);
      if (item) {
        clearTimeout(item.timer);
        pending.delete(m.id);
        if (m.error) item.reject(new Error('Typeless CDP 命令失败: ' + String(m.error.message || m.error.code || '未知错误').slice(0, 300)));
        else item.resolve(m);
      }
    };
    const send = (method, params) => new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        fail(new Error(`Typeless CDP 命令超时: ${method}`));
      }, 5000);
    ...
    const ev = async (expr) => {
      ...
      if (!r.result || typeof r.result !== 'object') throw new Error('Typeless CDP 执行结果无效');
      if (r.result.exceptionDetails) throw new Error('JS 错误: ' + (r.result.exceptionDetails.exception?.description?.slice(0, 300)));
      if (!r.result.result || typeof r.result.result !== 'object') throw new Error('Typeless CDP 执行结果无效');
      return r.result.result.value;
    };
    try {
      await new Promise((resolve, reject) => {
        rejectOpen = reject;
        connectTimer = setTimeout(() => fail(new Error('连接 Typeless WebSocket 超时')), 3000);
        ws.onopen = () => { clearTimeout(connectTimer); rejectOpen = null; resolve(); };
      });
      return await fn(send, ev);
    }
    finally {
      fail(new Error('Typeless CDP 连接已结束'));
      ws.onopen = ws.onmessage = null;
      if (typeof ws.terminate === 'function') {
        // ws 包在建连期间结束连接会异步触发 error;保留处理器直到 close 后再释放。
        const detach = () => { ws.onerror = ws.onclose = null; };
        ws.onerror = () => {};
        ws.onclose = detach;
        if (ws.readyState === 3) detach();
        else {
          try { ws.terminate(); } catch (_) { detach(); }
        }
      } else {
